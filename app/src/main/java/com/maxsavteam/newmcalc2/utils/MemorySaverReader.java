...
public class MemorySaverReader {
	...
	public void save(ArrayList<ListResult> results) {
		if(results == null){
	...
	public ArrayList<ListResult> read() {
		String memory = sp.getString( "memory", null );
		if ( memory == null ) {
			ArrayList<ListResult> results = new ArrayList<>( MEMORY_ENTRIES_COUNT );
			for (int i = 0; i < MEMORY_ENTRIES_COUNT; i++)
				results.add( ListResult.of( BigDecimal.ZERO ) );
			return results;
		}
		...
		try {
			...
			ArrayList<ListResult> results = new ArrayList<>();
